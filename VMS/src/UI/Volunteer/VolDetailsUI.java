 ...
 *
 */
public class VolDetailsUI extends JInternalFrame{

    private static final long serialVersionUID = 1L;
    mainWin fatherWin;  //设置父窗口
    private JTextField txtActName;    //小组名
    private JTextField txtActID;      //小组编号
    private JTextField txtSize;
    private JTextField txtHours;//小组限定人数
    private JTextField txtLeaderInfo;  //组长
    JTextArea txtProjectDesc;          //小组项目描述
    private JTable AllcomerTable;        //组员表格
    private JTable tblActivity;            //小组表格
    String [][] tableContent=null;       //二维数组，小组表格中内容
    DefaultTableModel allActivityModel;//小组表模型,与表结合显示
    TableColumn column;
    ActivityServices activityServices=null;//表列
    private JTextField txtAllTime;
    //逻辑层中的服务对象
    /**
     * 构造函数
     * @param f  用于与父窗体联系的对象
     */
    public VolDetailsUI(mainWin f) {
        ...
        panel_0.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "\u5DF2\u52A0\u5165\u5FD7\u613F\u6D3B\u52A8\u5FD7\u613F\u6D3B\u52A8", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
        ...
        scrollPane_1.setBounds(14, 31, 655, 184);
        ...
        panel_1.setBorder(new TitledBorder(null, "\u5FD7\u613F\u6D3B\u52A8\u8BE6\u60C5", TitledBorder.LEADING, TitledBorder.TOP, null, null));
        ...
        scrollPane.setBounds(14, 38, 404, 233);
        ...
        txtAllTime.setFont(new Font("微軟正黑體", Font.PLAIN, 18));
        txtAllTime.setBounds(743, 112, 119, 37);
        panel_0.add(txtAllTime);
        txtAllTime.setColumns(10);
        showAllActivity();

        JLabel lblNewLabel = new JLabel("\u5DF2\u5B8C\u6210\u5FD7\u613F\u65F6\u957F");
        lblNewLabel.setFont(new Font("微軟正黑體", Font.PLAIN, 18));
        lblNewLabel.setBounds(733, 65, 129, 37);
    ...
    private void showAllActivity() {
        String []tableHead=activityServices.getVolActTableHead();
        String [][]tableContent=activityServices.getVolActTableContent(fatherWin.getCurVol().getStuInfo().getStuID());
        allActivityModel=new DefaultTableModel(tableContent,tableHead);
        this.txtAllTime.setText(String.valueOf(activityServices.getVolAllTime(fatherWin.getCurVol().getStuInfo().getStuID())));
        this.tblActivity.setModel(allActivityModel);
